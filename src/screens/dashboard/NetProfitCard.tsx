...
const NetProfitCard = () => {
  return (
    ...
    transition={{ duration: 0.5, delay: 0.5 }} className='bg-componentBg  h-full rounded-md whitespace-nowrap'>
<div className='flex items-center justify-between  h-full w-full'>
    <div className='flex flex-col align-middle gap-4 xl:items-stretch h-full p-3 md:p-4  justify-between  lg:w-2/3'>

        <div className='text-sm font-normal'> {NetProfitCardDetail?.name}</div>
        <div className='text-4xl font-bold'> ₹{NetProfitCardDetail?.value}</div>
        <div className='text-sm text-[#12C290] flex items-center gap-1'>
        <ChevronUp className="" fill="#12C290" color="#12C290" />
          
          {NetProfitCardDetail?.percent} %</div>
